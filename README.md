...
- **工作模板**：新建空白視圖可選 **每日工作台（推薦）** 或 **專案進度看板**；既有看板可從工具列按 **模板** 重新套用。模板只改 Base 視圖設定，絕不改卡片或筆記 frontmatter。
- **新視圖不會壞**：還沒設定的視圖會顯示引導卡，分欄欄位打錯也只會出提示條，版面不會壞。
...
> 安全掃描可能將上述「全域狀態管理」標為 Vault Enumeration：這是因為外掛必須找出使用某個狀態的筆記與所有 `.base` 設定，才能安全地完成跨看板改名、換色與刪除。此列舉只在你主動執行該管理操作時發生，且不會傳送任何檔案內容或路徑到網路。
